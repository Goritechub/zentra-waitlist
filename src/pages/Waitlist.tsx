    ...
    try {
      const res = await fetch(`${API_BASE}/waitlist`, {
        ...
        body: JSON.stringify({
          ...
          project_description:
            role === "client" ? projectDescription.trim() || null : null,
        }),
      });
    ...
    <div className="min-h-screen bg-background relative overflow-hidden flex flex-col">
        ...
        <div className="max-w-2xl w-full text-center space-y-6 sm:space-y-8">
          ...
          <div className="space-y-3 sm:space-y-4 animate-fade-in">
            ...
            <p className="text-base sm:text-lg md:text-xl text-muted-foreground max-w-xl mx-auto leading-relaxed text-pretty">
              A marketplace connecting businesses with verified engineers,
              makers, and skilled fabricators for real-world technical projects.
              Join the waitlist to get early access.
            </p>
          </div>

          {/* Verified experts counter */}
          {waitlistCount > 0 && (
            <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground animate-fade-in">
              <Users className="h-4 w-4 text-primary" />
              <span>
                <span className="font-bold text-foreground">
                  {Math.floor(waitlistCount / 10) * 10}+
                </span>{" "}
                verified experts ready to work
              </span>
            </div>
          )}
          ...
          <div className="flex items-center justify-center gap-2 sm:gap-3 md:gap-5">
            {[
              ...
              <div key={item.label} className="flex flex-col items-center">
                ...
                <span className="text-[10px] sm:text-xs text-muted-foreground mt-1.5 sm:mt-2 font-medium">
                  {item.label}
                </span>
              </div>
            ))}
          ...
          {!joined ? (
            <form
              ...
              className="max-w-md mx-auto space-y-3 sm:space-y-4 text-left"
            >
              {/* Role selection */}
              <div className="grid grid-cols-2 gap-2 sm:gap-3">
                ...
                >
                  <Briefcase
                    className={`h-5 w-5 ${role === "client" ? "text-primary" : "text-muted-foreground"}`}
                  />
                  <span
                    className={`text-xs sm:text-sm font-semibold ${role === "client" ? "text-primary" : "text-foreground"}`}
                  >
                    I want to hire
                  </span>
                ...
                >
                  <Users
                    className={`h-5 w-5 ${role === "expert" ? "text-primary" : "text-muted-foreground"}`}
                  />
                  <span
                    className={`text-xs sm:text-sm font-semibold ${role === "expert" ? "text-primary" : "text-foreground"}`}
                  >
                    I'm an expert
                  </span>
                  <span className="text-[10px] sm:text-xs text-muted-foreground">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder={
                    role === "client"
                      ? "e.g. Construction, Oil & Gas"
                      : "e.g. AutoCAD, Civil Engineering"
                  }
            ...
            <div className="max-w-md mx-auto bg-primary/5 border border-primary/20 rounded-2xl p-6 sm:p-8 space-y-3 animate-scale-in">
              ...
              <p className="text-muted-foreground text-sm">
                We'll send you an email when ZentraGig is ready for beta. Stay
                tuned!
              </p>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 pt-4">
            {[
              ...
              },
              {
                Icon: Rocket,
                title: "Fast Matching",
                desc: "Get proposals from experts within hours",
              },
              {
                Icon: Trophy,
                title: "Contests",
                desc: "Launch creative contests and crowdsource solutions",
              },
            ].map((f) => (
              <div
                key={f.title}
                className="bg-card border border-border rounded-xl p-4 sm:p-5 text-center space-y-1.5 sm:space-y-2 hover-scale"
              >
      ...
      <footer className="relative z-10 text-center py-5 sm:py-6 space-y-3 border-t border-border/50">
        <div className="flex items-center justify-center gap-4">
          ...
          <a
            href="https://www.youtube.com/@zentragig"
            target="_blank"
            rel="noopener noreferrer"
            className="p-2 rounded-lg border border-border bg-card hover:bg-accent transition-colors hover-scale"
